...
#ifdef CFG_CAAM_SEPARATE_BUFFER
static vaddr_t caam_buffer1, caam_buffer2;
static vaddr_t overlay_buffer;
static int caam_buffer_index;
#endif


uint32_t jr_id = 1;
bool dequeue_background = false;
	...
	for (; nb_jobs_done; nb_jobs_done--) {
		...
		for (idx_jr = 0, found = false; idx_jr < jr_privdata->nb_jobs;
			...
			if (caam_desc_pop(&jr_out->desc) == caller->pdesc) {
                ...
                //paddr_t p_addr = (paddr_t) virt_to_phys((void *) &jr_out->status);

				/* Update return Job IDs mask */
				if (caller->job_id & wait_job_ids)
					ret_job_id |= caller->job_id;
                
        //        EMSG("caller->job_id: %d, ret_job_id: %d ", caller->job_id, ret_job_id);
...
#ifdef CFG_CAAM_SEPARATE_BUFFER
                void *decrypted;
                if (caam_buffer_index == 0)                
                    decrypted = caam_buffer1;
                else
                    decrypted = caam_buffer2;
                caam_buffer_index = caam_buffer_index ^ 1;
                ...
                for (row = 0; row < yRes; row++) 
                    memcpy(overlay_buffer + (row * row_size), 
...
enqueue:
	...
	if (!found) {
        ...
        //EMSG("Error didn't find a free space in the callers array.");

        int nb_jobs_done = caam_hal_jr_get_nbjob_done(jr_privdata->baseaddr);
        if (nb_jobs_done != 0) {
            uint32_t id;
            uint32_t job_complete = do_jr_dequeue(id);
            JR_TRACE("    - # of jobs dequeued: %d", nb_jobs_done);
           // EMSG("job_complete: %d, nb_jobs_done: %d", job_complete, nb_jobs_done);
            /*
            ...
            for (row = 0; row < 800; row++) 
                ...
                */
            /*
            int nb_slots_available = caam_hal_jr_read_nbslot_available(jr_privdata->baseaddr);
            int nb_jobs_done = io_caam_read32(jr_privdata->baseaddr);

            EMSG("[Waited] nb_slots_available: %d, nb_jobs_done: %d", nb_slots_available, nb_jobs_done);
*/
        }
...
/*
    EMSG("ack_itr");
    EMSG(" - %d", caam_hal_jr_check_ack_itr(jr_privdata->baseaddr));
    EMSG(" - %d", caam_hal_jr_check_ack_itr(jr_privdata->baseaddr));
    EMSG(" - %d", caam_hal_jr_check_ack_itr(jr_privdata->baseaddr));
*/
/*
    int nb_slots_available = caam_hal_jr_read_nbslot_available(jr_privdata->baseaddr);
    int nb_jobs_done = caam_hal_jr_get_nbjob_done(jr_privdata->baseaddr);
    EMSG("nb_slots_available: %d, nb_jobs_done: %d", nb_slots_available, nb_jobs_done);
    
    
    while (nb_slots_available == 0 || nb_jobs_done > 0) {
        if (!caam_hal_jr_check_ack_itr(jr_privdata->baseaddr))
			caam_udelay(10);
*/
    ...
    //EMSG("job_completed: %"PRIu32", id: %d", job_complete, id);
        
        /* Update # of available slots and # of jobs done*/ 
        /*
        nb_slots_available = caam_hal_jr_read_nbslot_available(jr_privdata->baseaddr);
        nb_jobs_done = caam_hal_jr_get_nbjob_done(jr_privdata->baseaddr);
    }
    ...
    */
    ...
    //if (job_id)
    //    EMSG("job_id: 0x%p, *job_id: %d", job_id, *job_id);
...
enum caam_status caam_jr_init(struct caam_jrcfg *jrcfg)
{
    ...
    paddr_t p_baseaddr = (paddr_t) virt_to_phys((void *) jr_privdata->baseaddr);
    //EMSG("jr_privdata->baseaddr: %p\n", p_baseaddr);

#ifdef CFG_CAAM_SEPARATE_BUFFER
    overlay_buffer = phys_to_virt(CFG_FBMEM_START, MEM_AREA_IO_SEC);
    caam_buffer1 = phys_to_virt(CFG_FBMEM_START + 0x400000, MEM_AREA_IO_SEC);
    caam_buffer2 = phys_to_virt(CFG_FBMEM_START + 0x600000, MEM_AREA_IO_SEC);
    caam_buffer_index = 0;
#endif
...
/* When running asynchronous, flush all the completed jobs in the job slots */
void caam_jr_dequeue_all(void) {
    
    uint32_t id;
    int nb_slots_available = caam_hal_jr_read_nbslot_available(jr_privdata->baseaddr);
    paddr_t p_addr = (paddr_t) virt_to_phys((void *) &jr_privdata->baseaddr);
    
    while (nb_slots_available != 10) {
        uint32_t job_completed = do_jr_dequeue(id);
        JR_TRACE("job_completed: %d", job_completed);
        nb_slots_available = caam_hal_jr_read_nbslot_available(jr_privdata->baseaddr);
    }
...
/* When running asynchronous, dequeue completed jobs in background */
void caam_jr_dequeue_in_background(bool enable) {
    
    uint32_t job_completed, id;
    dequeue_background = enable;
    
    while (dequeue_background) {
        job_completed = do_jr_dequeue(id);
