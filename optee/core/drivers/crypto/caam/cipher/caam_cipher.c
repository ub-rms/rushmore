 ...
 *  0: Disable, 1: Enable
 */
#define ENABLE_ASYNCH 1

/*
 *  0: Disable, 1: Enable
 */
#define ENABLE_DEQUEUE_IN_BACKGROUND 0 
...
/*
 *  Callback function for running asynchronous CAAM functions
 *
 */
static void caam_done(struct caam_jobctx *jobctx)
{
    //EMSG("CAAM Done. ID: 0x%08" PRIx32 " with status 0x%" PRIx32, 
    //        jobctx->id, jobctx->status);
    return;
    //free(jobctx);
}
...
handle_context:
    ...
    if (ENABLE_ASYNCH) {
        jobctx.callback = caam_done;
        //uint32_t *job_id = malloc(sizeof(uint32_t));
        //*job_id = 1;
        JR_TRACE("job_id: %d", *job_id);
        retstatus = caam_jr_enqueue(&jobctx, job_id);
        *job_id = (*job_id + 1) % 10;
...
static TEE_Result do_init(struct drvcrypt_cipher_init *dinit)
{
    ...
    *job_id = 0;
    ...
    /* Enable background dequeueing */ 
    if (ENABLE_DEQUEUE_IN_BACKGROUND) {
        initialized = true;
        bool enable = true;
        caam_jr_dequeue_in_background(enable);
    }
...
static TEE_Result do_update_streaming(struct drvcrypt_cipher_update *dupdate)
{
	...
	if (size_todo) {
		...
		if (retstatus != CAAM_NO_ERROR && !ENABLE_ASYNCH) {
			ret = TEE_ERROR_GENERIC;
			goto out;
		}
...
static TEE_Result do_update(struct drvcrypt_cipher_update *dupdate)
{
    ...
    */
    ...
    TEE_Result res = cipherdata->alg->update(dupdate);
...
{
    /* Diable the background dequeueing thread */
    initialized = false; 
    bool enable = false;
    if (!ENABLE_DEQUEUE_IN_BACKGROUND) 
        caam_jr_dequeue_in_background(enable);
